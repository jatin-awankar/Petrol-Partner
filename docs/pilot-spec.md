...
Students require verified email ownership, current enrollment evidence, age eligibility, and operator approval. Phone ownership verification is not part of this pilot and no phone number is an eligibility condition. Offering or starting a ride additionally requires current approval of the driver, the selected car, and that driver-car association. A car approval records expiry or review date, status, reviewer, and audit history. Revocation takes effect immediately for new protected actions.
...
Any optional phone number or emergency contact supplied by a student is unverified, private, and never presented as proof of identity or shared with other participants. The pilot does not send verification codes or SMS. Removing or changing an optional number does not alter ride eligibility. Confirmed-trip coordination uses the platform's trip details and durable in-app/email notices; the published operator support contact handles exceptions during operating windows. The launch rehearsal must show that participants can coordinate pickup without relying on another participant's phone number.
...
Confirmed participants in the same ride may see verified names, pickup and departure details, and limited car identification required for coordination. No participant phone number is exposed to another participant in the pilot, including after confirmation. Ordinary trip-detail access ends 24 hours after completion or cancellation. Optional phone numbers, emergency contacts, raw documents, and sensitive identifiers stay out of logs, analytics, notification URLs, and caches. Operator access to private contact data is limited to documented support or incident purposes and audited.
