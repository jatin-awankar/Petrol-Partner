...
export default function EligibilityPage() {
  ...
  const [ageCategory, setAgeCategory] = useState<(typeof ageCategories)[number]>("institution_age_record");
  const [enrollmentFile, setEnrollmentFile] = useState<File | null>(null);
  const [ageFile, setAgeFile] = useState<File | null>(null);
  const [replacementPurpose, setReplacementPurpose] = useState<"enrollment" | "age">("enrollment");
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      if (!enrollmentFile || !ageFile) throw new Error("Choose both evidence files.");
      if (evidenceMode === "closed") throw new Error("Evidence intake is closed.");
      await apiRequest("/v1/verification/student", { method: "PUT", body: JSON.stringify({ provider: "manual_review", enrolled_name: name, institution_name: institution, admission_year: admissionYear, graduation_year: graduationYear, evidence_category: category, age_evidence_category: ageCategory }) });
      await apiRequest("/v1/verification/student/evidence?purpose=enrollment", { method: "POST", headers: { "Content-Type": enrollmentFile.type, ...(evidenceMode === "synthetic" ? { "X-Synthetic-Evidence": "true" } : {}) }, body: enrollmentFile, skipJsonBody: true });
      await apiRequest("/v1/verification/student/evidence?purpose=age", { method: "POST", headers: { "Content-Type": ageFile.type, ...(evidenceMode === "synthetic" ? { "X-Synthetic-Evidence": "true" } : {}) }, body: ageFile, skipJsonBody: true });
      setMessage("Evidence submitted for operator review.");
      await refresh();
  ...
  async function replaceLostEvidence(event: React.FormEvent) {
    event.preventDefault();
    const file = replacementPurpose === "enrollment" ? enrollmentFile : ageFile;
    if (!file) { setMessage("Choose an evidence file."); return; }
    setBusy(true);
    try {
      await apiRequest(`/v1/verification/student/evidence?purpose=${replacementPurpose}`, { method: "POST", headers: { "Content-Type": file.type, ...(evidenceMode === "synthetic" ? { "X-Synthetic-Evidence": "true" } : {}) }, body: file, skipJsonBody: true });
      setMessage("Replacement evidence received.");
  ...
  return <main className="mx-auto max-w-2xl space-y-6 p-8">
    ...
    {review?.status === "pending_review" && <form onSubmit={replaceLostEvidence} className="space-y-3 rounded border p-4">
      ...
      <p>Use this only if the operator reports that your document is missing. An available document cannot be replaced.</p>
      <select value={replacementPurpose} onChange={(event) => setReplacementPurpose(event.target.value as typeof replacementPurpose)}><option value="enrollment">Enrollment</option><option value="age">Age</option></select>
      <input type="file" accept="image/jpeg,image/png,application/pdf" required onChange={(event) => replacementPurpose === "enrollment" ? setEnrollmentFile(event.target.files?.[0] ?? null) : setAgeFile(event.target.files?.[0] ?? null)} />
      <button disabled={busy || evidenceMode === "closed"}>Upload replacement</button>
    </form>}
    {(!review || review.status === "rejected") && <form onSubmit={submit} className="space-y-3 rounded border p-4">
      <h2 className="font-semibold">{evidenceMode === "synthetic" ? "Synthetic submission" : "Eligibility submission"}</h2>
      ...
      <label className="block">Age evidence category<select className="block w-full border p-2" value={ageCategory} onChange={(event) => setAgeCategory(event.target.value as typeof ageCategory)}>{ageCategories.map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label>
      <label className="block">{evidenceMode === "synthetic" ? "Fabricated " : ""}Enrollment document (JPEG, PNG or PDF; 512 KB maximum)<input className="block w-full" type="file" accept="image/jpeg,image/png,application/pdf" required onChange={(event) => setEnrollmentFile(event.target.files?.[0] ?? null)} /></label>
      <label className="block">{evidenceMode === "synthetic" ? "Fabricated " : ""}Redacted age document (JPEG, PNG or PDF; 512 KB maximum)<input className="block w-full" type="file" accept="image/jpeg,image/png,application/pdf" required onChange={(event) => setAgeFile(event.target.files?.[0] ?? null)} /></label>
      <button className="rounded bg-black px-4 py-2 text-white" disabled={busy || evidenceMode === "closed"}>{busy ? "Submitting…" : "Submit evidence"}</button>
    </form>}
  </main>;
}
